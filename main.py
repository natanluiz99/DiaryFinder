...
from transformers import pipeline, TFBertForQuestionAnswering
import fitz  # PyMuPDF
...
# Inicializar o modelo BERT para question answering
model = TFBertForQuestionAnswering.from_pretrained("neuralmind/bert-large-portuguese-cased", from_pt=True)
qa_pipeline = pipeline("question-answering", model=model, tokenizer="neuralmind/bert-large-portuguese-cased")
...
async def analyze_pdf(file: UploadFile = File(...), question: str = None):
    try:
        ...
        if question:
            ...
            return {"error": "Por favor, forneça uma pergunta."}
